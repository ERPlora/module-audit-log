"""Tests for audit_log views."""
import pytest
from django.urls import reverse


@pytest.mark.django_db
class TestDashboard:
    """Dashboard view tests."""

    def test_dashboard_loads(self, auth_client):
        """Test dashboard page loads."""
        url = reverse('audit_log:dashboard')
        response = auth_client.get(url)
        assert response.status_code == 200

    def test_dashboard_htmx(self, auth_client):
        """Test dashboard HTMX partial."""
        url = reverse('audit_log:dashboard')
        response = auth_client.get(url, HTTP_HX_REQUEST='true')
        assert response.status_code == 200

    def test_dashboard_requires_auth(self, client):
        """Test dashboard requires authentication."""
        url = reverse('audit_log:dashboard')
        response = client.get(url)
        assert response.status_code == 302


@pytest.mark.django_db
class TestAuditEntryViews:
    """AuditEntry view tests."""

    def test_list_loads(self, auth_client):
        """Test list view loads."""
        url = reverse('audit_log:audit_entries_list')
        response = auth_client.get(url)
        assert response.status_code == 200

    def test_list_htmx(self, auth_client):
        """Test list HTMX partial."""
        url = reverse('audit_log:audit_entries_list')
        response = auth_client.get(url, HTTP_HX_REQUEST='true')
        assert response.status_code == 200

    def test_list_search(self, auth_client):
        """Test list search."""
        url = reverse('audit_log:audit_entries_list')
        response = auth_client.get(url, {'q': 'test'})
        assert response.status_code == 200

    def test_list_sort(self, auth_client):
        """Test list sorting."""
        url = reverse('audit_log:audit_entries_list')
        response = auth_client.get(url, {'sort': 'created_at', 'dir': 'desc'})
        assert response.status_code == 200

    def test_export_csv(self, auth_client):
        """Test CSV export."""
        url = reverse('audit_log:audit_entries_list')
        response = auth_client.get(url, {'export': 'csv'})
        assert response.status_code == 200
        assert 'text/csv' in response['Content-Type']

    def test_export_excel(self, auth_client):
        """Test Excel export."""
        url = reverse('audit_log:audit_entries_list')
        response = auth_client.get(url, {'export': 'excel'})
        assert response.status_code == 200

    def test_add_form_loads(self, auth_client):
        """Test add form loads."""
        url = reverse('audit_log:audit_entry_add')
        response = auth_client.get(url)
        assert response.status_code == 200

    def test_add_post(self, auth_client):
        """Test creating via POST."""
        url = reverse('audit_log:audit_entry_add')
        data = {
            'action': 'New Action',
            'entity_type': 'New Entity Type',
            'entity_id': 'test',
            'user_id': 'test',
            'user_name': 'New User Name',
        }
        response = auth_client.post(url, data)
        assert response.status_code == 200

    def test_edit_form_loads(self, auth_client, audit_entry):
        """Test edit form loads."""
        url = reverse('audit_log:audit_entry_edit', args=[audit_entry.pk])
        response = auth_client.get(url)
        assert response.status_code == 200

    def test_edit_post(self, auth_client, audit_entry):
        """Test editing via POST."""
        url = reverse('audit_log:audit_entry_edit', args=[audit_entry.pk])
        data = {
            'action': 'Updated Action',
            'entity_type': 'Updated Entity Type',
            'entity_id': 'test',
            'user_id': 'test',
            'user_name': 'Updated User Name',
        }
        response = auth_client.post(url, data)
        assert response.status_code == 200

    def test_delete(self, auth_client, audit_entry):
        """Test soft delete via POST."""
        url = reverse('audit_log:audit_entry_delete', args=[audit_entry.pk])
        response = auth_client.post(url)
        assert response.status_code == 200
        audit_entry.refresh_from_db()
        assert audit_entry.is_deleted is True

    def test_bulk_delete(self, auth_client, audit_entry):
        """Test bulk delete."""
        url = reverse('audit_log:audit_entries_bulk_action')
        response = auth_client.post(url, {'ids': str(audit_entry.pk), 'action': 'delete'})
        assert response.status_code == 200
        audit_entry.refresh_from_db()
        assert audit_entry.is_deleted is True

    def test_list_requires_auth(self, client):
        """Test list requires authentication."""
        url = reverse('audit_log:audit_entries_list')
        response = client.get(url)
        assert response.status_code == 302


@pytest.mark.django_db
class TestSettings:
    """Settings view tests."""

    def test_settings_loads(self, auth_client):
        """Test settings page loads."""
        url = reverse('audit_log:settings')
        response = auth_client.get(url)
        assert response.status_code == 200

    def test_settings_requires_auth(self, client):
        """Test settings requires authentication."""
        url = reverse('audit_log:settings')
        response = client.get(url)
        assert response.status_code == 302

